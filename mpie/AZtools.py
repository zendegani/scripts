...
def SFE_chu(E_C14, E_C15, a_C14):
    ...
    return [
        24 * Edelta_15_14 / (3**0.5 * a_C14**2) * _conv["meV_ang2__mJ_m2"],
        Edelta_15_14,
    ]
...
class projects_path_keeper:
    ...
    def db_load(self):
        try:
            ...
            print(("Database not found! Create a new one with .db_create()", e.message))
        else:
            print("database of projects' path loaded.")

    def db_save(self):
        json.dump(
            self.data, open(op.join(self.wk_dir, self.db_filename), "w"), sort_keys=True
        )
...
class MatPhaseObj:
    ...
    def __init__(self, name, project_dict, def_name=None):
        ...
        self._flag_OSZICAR = json.load(open(self.path_dbs + "_flag_OSZICAR.txt"))
        ...
        self._file_exist = {
            "Harmonic": False,  # the file thermo.out which contains harmonic free energy
            "background": False,
            "Murn": False,
            "EV": False,
            "staticOUT": False,
        ...
        self._folder_exist = {
            "harmonic": False,  # the main folder contains displaced structures and forces_background
            "thermo": False,
            "background": False,
            "Murn": False,
            "static": False,
    ...
    def getFileName(path, pattern):
        ...
        if not _:
            ...
            print(
                (
                    "Error: more than one file found starting wiht",
                    pattern,
    ...
    def data_extractor_OSZICAR(self, OSZICAR_path):
        ...
        _fileOSZICAR = MatPhaseObj.getFileName(OSZICAR_path, self._file_name["OSZICAR"])
        if _fileOSZICAR:
        ...
        if self._file_exist["Murn"]:
            ...
            for i, line in enumerate(fp):
                if i == 1:
                    ...
                    self.murnV0angst = self.murnV0bohr * self._conv["vol_bohr2angst"]
                elif i > 5:
                    break
            fp.close()
    ...
    def data_extractor_OUTCAR(self, OUTCARfile):
        ...
        for line in _:
            if self._flag_OUTCAR["energy"] in line:
                output["E_Sigma0_perCell_eV"] = float(line.split()[-1])  # eV per cell
        ...
        output["E_Sigma0_perAtom_eV"] = (
            output["E_Sigma0_perCell_eV"] / output["AtomTotalNum"]
        )  # eV per atom
        ...
        for line in reversed(_):
            if "magnetization (x)" in line:
                is_ymag = (
                    "magnetization (y)" in _[-counter - 1 + output["AtomTotalNum"] + 9]
                )
                is_zmag = (
                    "magnetization (z)"
                    in _[-counter - 1 + 2 * (output["AtomTotalNum"] + 9)]
                )
                for i in range(output["AtomTotalNum"]):
                    output["MagX"][i] = np.asarray(
                        list(map(float, _[-counter - 1 + 4 + i].split()))
                    )  # x
                    if is_ymag:
            ...
            counter += 1
        if (
            np.sum(np.nonzero(np.array(output["MagX"][:, -1]) != 0)) > 1
            or np.sum(np.nonzero(np.array(output["MagY"][:, -1]) != 0)) > 1
            or np.sum(np.nonzero(np.array(output["MagZ"][:, -1]) != 0)) > 1
        ):
    ...
    def loadData(self):
        ...
        if self._file_exist["staticOUT"]:
            ...
            self.static["penalty"], self.static["Lambda"] = self.data_extractor_OSZICAR(
                self.project_dict["static"]["path"]
            )
        if self._file_exist["background"]:
            self.background = self.data_extractor_OUTCAR(self._fileOUTCARbackground)
            self.background["penalty"], self.background["Lambda"] = (
                self.data_extractor_OSZICAR(
                    op.join(
        ...
        try:
            (
                self.d_tot_ene_per_atom_eV,
                self.d_tot_penalty_ene_eV,
                self.d_lambda,
                self.d_tot_vol_atom_ang3,
                self.d_tot_atom_num,
                self.d_MagX,
                self.d_MagY,
                self.d_MagZ,
            ) = (
                self.static["E_Sigma0_perAtom_eV"],
                self.static["penalty"],
                self.static["Lambda"],
                self.static["vol_atom"],
                self.static["AtomTotalNum"],
                self.static["MagX"],
                self.static["MagY"],
                self.static["MagZ"],
            )
            ...
            try:
                (
                    ...
                    self.background["E_Sigma0_perAtom_eV"],
                    self.background["penalty"],
                    self.background["Lambda"],
                    self.background["vol_atom"],
                    self.background["AtomTotalNum"],
                    self.background["MagX"],
                    self.background["MagY"],
                    self.background["MagZ"],
                )
            except:
                try:
                    (
    ...
    def verifyInput(self):
        ...
        if self.project_dict["harmonic"]["path"]:
            self.project_dict["harmonic"]["path"] = op.join(
                self.project_dict["harmonic"]["path"], ""
            )
            if op.isdir(self.project_dict["harmonic"]["path"]):
                self._folder_exist["harmonic"] = True
                if op.isdir(
                    self.project_dict["harmonic"]["path"] + self._folder_name["thermo"]
                ):
        ...
        if self.project_dict["murn"]["path"]:
            ...
            if op.isdir(self.project_dict["murn"]["path"]):
                ...
                self._fileMurn = (
                    self.project_dict["murn"]["path"] + self._file_name["Murn"]
                )
                if op.isfile(self._fileMurn):
                    self._file_exist["Murn"] = True
                self._fileEV = self.project_dict["murn"]["path"] + self._file_name["EV"]
            ...
            if self.project_dict["static"]["path"]:
                ...
                if op.isdir(self.project_dict["static"]["path"]):
                    ...
                    self._fileOUTCAR = MatPhaseObj.getFileName(
                        self.project_dict["static"]["path"], self._file_name["OUTCAR"]
                    )
