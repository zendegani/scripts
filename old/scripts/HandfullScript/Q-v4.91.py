...
""" unsolved issue indicated by !!!!
    many space required in 1st & 6th line of POS.0 !!!! sloved
...
class SiteFrac(object):
    ...
    def EfitPol(self,filename,degree):
        """ read file contains free energy vs temperature
        and fit a polynomial function of 13 degree to free energy
        """
        dat = np.loadtxt(filename);
    ...
    def fit4TCM(self):
        ...
        for i in range(self.nC):
            ...
            Tstring = 'ent_par G('+self.phase+','+':'.join(list(self.Comb[i])[1:self.nM+1]).upper()+ \
                ':'+':'.join(self.Elfixed).upper()+') 298.15 '+ Tstring
            TCM.write(Tstring+'\n');
            #print(Tstring+'\n')
        TCM.close();
        return
...
#ax.set_xlabel('T(K)');
#ax.set_ylabel('Free energies of formation '+ur'$F^{vib}_{harm}$'+' (eV/atom)');
