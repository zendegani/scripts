...
#        print 'Table of elements              : ', self.El;
    ...
    """
        ...
        while conditionModel:
            ...
            self.sublatModel=[]
            for i in self.ElWykList:
                self.sublatModel.append(','.join(j for j in i));
            print ':'.join(i for i in ["({}){}".format(a_, b_) for a_, b_ in zip(self.sublatModel,self.M)]);
            happy=raw_input('Are you happy with the above model [yes]/no? ').upper() or "YES"
            if happy == "YES" : conditionModel = False;
...
#        if self.ModeVerbose == 'YES': print np.array(self.PermRep), '\n\n';
    ...
    """
        self.TabExt = [([i + 1] + [self.PermRep[i][j] for j in range(self.nM)] + [0.000 for                                          j in range(self.nElements)] + [0]) for i in range(self.nC)];
        ...
        for line in fileConf:
            ...
            for i in range(self.nElements):
                if float(line.split()[1 + self.nM + i]) != 0.0:
                    filePOTel = open(self.POTPATH+'/' + self.elementSelected[i]+'/POTCAR', 'r');
    ...
    def Composition(self):
        ...
        for i in range(self.nC):
            ...
            for el in self.elementSelected:
                ...
                nnl.append(nl);
#            nnl += self.ElfixedNo;
            self.compositionTab.append(list(self.Comb[i])[1:self.nM+1]+nnl+self.elementSelected);
            print(nnl)
        return
    ...
    def fit4TCM(self):
        ...
        for i in range(self.nC):
	      ...
	      if self.GofQ4TCM[i][d+1] < 0 :
                ...
                Tstring += '+' + str(ntot*self.GofQ4TCM[i][d+1]) + '*T**' + str(d+1)
            Tstring = 'ent_par G('+self.phase+','+':'.join(list(self.Comb[i])[1:self.nM+1]).upper()+                 ':'+') 298.15  \n'+ Tstring + '\n \n6000 \nN ;,,,,,,,Q  J/mol from VASP \n \n';
