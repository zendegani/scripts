...
import mpld3
mpld3.enable_notebook() #http://stackoverflow.com/questions/10655217/ipython-notebook-pylab-inline-zooming-of-a-plot
...
class SiteFrac(object):
    def __init__(self, phase, ElementList, ElNumber, NvarEl, ModeVerbose='YES', ModeFile='NO',wkpath="./"):
        self.wkpath=wkpath;
        self.phase = phase;
    ...
    """
        self.M = [];
        nPOS = int(self.subp("ls -1d "+self.wkpath+"POS/POS.[1-9]* | wc -l"));
        for i in range(1, nPOS + 1):
            num_lines = int(self.subp('cat '+self.wkpath+'POS/POS.' + str(i) + ' | wc -l'));
            #      num_lines = sum(1 for line in open('POS/'+'POS.'+str(i)));
            self.M.append(num_lines);
        print "results stored in .M"
    ...
    """
        ...
        print "results stored in .PermRep"


    def TabExt(self):
        """ Extend table size to provide space for composition
    """
        self.TabExt = [([i + 1] + [self.PermRep[i][j] for j in range(self.nM)] + [0.000 for                       j in range(self.nEl)] + [0]) for i in range(self.nC)];
        if self.ModeVerbose == 'YES': print 'Extended table \n', np.array(self.TabExt), '\n\n';
        print "results stored in .TabExt"
    ...
    """
        ...
        for i in range(1, self.nC + 1):
            self.subp('mkdir -p ' + self.wkpath + self.name + '/' + str(i));
    ...
    """
        ...
        f.close();
        print "results are saved in file 'conf.out'"
    ...
    """
        ...
        # preparing header
        for i in range(self.nC):
            folder = self.wkpath+self.name + '/' + str(i + 1)
            self.subp('cp '+self.wkpath+'POS/POS.0 ' + folder + '/.')
            ...
            for el in self.El:
                for k in range(self.nM):
                    if el == self.Comb[i][k + 1]:
                        self.subp('cp '+self.wkpath+'POS/POS.' + str(k + 1) + ' ' + folder + '/.')
            ...
            self.subp('cp '+self.wkpath+'POS/POS.other ' + folder + '/.')
            filePOSother = open(folder + '/POS.other', 'r+')
            filePOSCAR.write(filePOSother.read());
            filePOSother.close();
            filePOSCAR.close();
        print "POSCARs are generated!"
    ...
    """
        ...
        for line in fileConf:
            ...
            filePOT = open(self.wkpath+self.name + '/' + str(folder) + '/POTCAR', 'w');
            for i in range(self.nEl):
                if float(line.split()[1 + self.nM + i]) != 0.0:
                    filePOTel = open(self.wkpath+'POT/POTCAR_' + self.El[i], 'r');
                    filePOT.write(filePOTel.read());
                    #filePOT.write('\n');
                    filePOTel.close();

            for i in range(len(self.Elfixed)):
                filePOTel = open(self.wkpath+'POT/POTCAR_'+self.Elfixed[i], 'r');
                filePOT.write(filePOTel.read());
                #filePOT.write('\n');
                filePOTel.close();
            filePOT.close();
        fileConf.close();
        print "POTCARS are generated!"
    ...
    def Composition(self):
        ...
        for i in range(self.nC):
            ...
            print(nnl)
        print "results stored in .compositionTab"            
        return
    ...
    def EofQs(self):
        ...
        for el in self.Elements:
            ...
            self.T0ofEl.append(np.loadtxt(self.wkpath+'FreeEneHarmonic/T0_'+el));
            #Harmonic Contribution of Free Energy
            self.GofEl.append(self.EfitPol(self.wkpath+'FreeEneHarmonic/Fvib_'+el,fittingDeg));
        for i in range(self.nC):
            ...
            self.T0ofQ.append(1000*float(np.array(self.file2dat(self.wkpath+self.name+                                                                '/sum.out'))[i+1].split()[self.nM+2]));
            self.GofQ.append(self.EfitPol(self.wkpath+self.name+'/'+str(i+1)+'/finite/thermo/thermo.out',fittingDeg));
            thermomeV=open(self.wkpath+'FreeEneHarmonic/thermomeV.'+str(i+1),'w');
            thermoJ=open(self.wkpath+'FreeEneHarmonic/thermoJ.'+str(i+1),'w');
            for T in range(1000):
                G = 0;
            ...
            thermomeV.close();
            thermoJ.close();
        print "E0 of elements:", '\n', self.T0ofEl , '\n', "E0 of end-members" , '\n',self.T0ofQ;
        print "results are saved in 'thermomeV.#' and 'thermoJ.#'"
        return    
    ...
    def fit4TCM(self):
        ...
        '''
        degree = 15;
        TCM = open(self.wkpath+'TCMparam.txt','w');
        ntot = np.sum(self.compositionTab[0][self.nElements:self.nElements*2]);
        for i in range(self.nC):
            file = self.wkpath+'FreeEneHarmonic/thermoJ.'+str(i+1);
            ...
            for d in range(degree):
                if self.GofQ4TCM[i][d+1] < 0 :
                    ...
                    Tstring += '+' + str(ntot*self.GofQ4TCM[i][d+1]) + '*T**' + str(d+1)
            Tstring = 'ent_par G('+self.phase+','+':'.join(list(self.Comb[i])[1:self.nM+1]).upper()+                 ':'+':'.join(self.Elfixed).upper()+') 298.15  \n'+ Tstring +                '\n \n6000 \nN ;,,,,,,,Q  J/mol from VASP \n \n'; # '+ Tstring + ';,,,,,,,Q  J/mol from VASP';
            TCM.write(Tstring+'\n');
            #print(Tstring+'\n')
        TCM.close();
        return
    ...
    def EofQsFixedRef(self):
        ...
        for i in range(self.nC):
            ...
            self.T0ofQ.append(1000*float(np.array(self.file2dat(self.wkpath+                                                                self.name+'/sum.out'))[i+1].split()[self.nM+2]));
            self.GofQ.append(self.EfitPol(self.wkpath+self.name+'/'+str(i+1)+'/finite/thermo/thermo.out',fittingDeg));
            F_meV_FixRef_Formula=open(self.wkpath+'FreeEneHarmonic/F_meV_FixRef_Formula.'+str(i+1),'w');
            F_J_FixRef_Formula=open(self.wkpath+'FreeEneHarmonic/F_J_FixRef_Formula.'+str(i+1),'w');
            F_meV_FixRef=open(self.wkpath+'FreeEneHarmonic/F_meV_FixRef.'+str(i+1),'w');
            F_J_FixRef=open(self.wkpath+'FreeEneHarmonic/F_J_FixRef.'+str(i+1),'w');
            for T in range(1000):
                G = 0;
                for j in range(self.nElements):
    ...
    def fit4TCMfixedRef(self):
        ...
        '''
        degree = 15;
        TCM = open(self.wkpath+'TCMparamFixedRef.txt','w');
        for i in range(self.nC):
            file = self.wkpath+'FreeEneHarmonic/F_J_FixRef_Formula.'+str(i+1);
            self.GofQ4TCM.append(self.EfitPol(file,degree));
            # PARAMETER G(LIQUID,CR;0) 2.98140E+02 +24339.955-11.420225*T+2.37615E-21*T**7
            # ent_par G(Q,AL:AL:AL:AL:CU:SI) 298.15 9234.1184
    ...
    def EofQsFixedRef298(self):
        ...
        for el in self.Elements:
            ...
            self.T0ofEl.append(np.loadtxt(self.wkpath+'FreeEneHarmonic/T0_'+el));
            #Harmonic Contribution of Free Energy
            GofEl_tmp= np.loadtxt(self.wkpath+'FreeEneHarmonic/Fvib_'+el);
            if 298 in GofEl_tmp[:,0]:   #check whether T=298K is in file or should interpolated!
                ...
                self.GofEl.append(self.EfitPol(self.wkpath+'FreeEneHarmonic/Fvib_'+el,fittingDeg)(298));
        print 
        for i in range(self.nC):
            #Total E0 of each Q provided in eV/atom (VASP --> murn --> converted from Hartree to eV/atom)
            #1st row of sum.out is skipped by using [i+1]
            self.T0ofQ.append(1000*float(np.array(self.file2dat(self.wkpath+self.name+'/sum.out'))[i+1].split()[self.nM+2]));
            self.GofQ.append(np.loadtxt(self.wkpath+self.name+'/'+str(i+1)+'/finite/thermo/thermo.out'));
            ...
            np.savetxt(self.wkpath+'FreeEneHarmonic/F_meV_FixRef_Formula298.'+str(i+1),Gcompound[:,0:2])

            Gcompoundtmp=Gcompound.copy();
            Gcompoundtmp[:,1]=Gcompound[:,1]*self.meV2J;
            #F_J_FixRef_Formula298.write(str(Gcompoundtmp));
            np.savetxt(self.wkpath+'FreeEneHarmonic/F_J_FixRef_Formula298.'+str(i+1),Gcompoundtmp[:,0:2])

            Gcompoundtmp=Gcompound.copy();
            Gcompoundtmp[:,1]=Gcompound[:,1]/ntot;
            #F_meV_FixRef298.write(str(Gcompoundtmp));
            np.savetxt(self.wkpath+'FreeEneHarmonic/F_meV_FixRef298.'+str(i+1),Gcompoundtmp[:,0:2])

            Gcompoundtmp=Gcompound.copy();
            Gcompoundtmp[:,1]=Gcompound[:,1]*self.meV2J/ntot;
            #F_J_FixRef298.write(str(Gcompoundtmp));
            np.savetxt(self.wkpath+'FreeEneHarmonic/F_J_FixRef298.'+str(i+1),Gcompoundtmp[:,0:2])

            #F_meV_FixRef_Formula298.close();
            #F_J_FixRef_Formula298.close();
            #F_meV_FixRef298.close();
            #F_J_FixRef298.close();
            print 'test', G, G0, Gcompound[0,1];
        print self.T0ofEl , '\n', self.T0ofQ, '\n', self.GofEl ;
        return
    ...
    def fit4TCMfixedRef298(self):
        ...
        '''
        degree = 15;
        TCM = open(self.wkpath+'TCMparamFixedRef298.txt','w');
        for i in range(self.nC):
            file = self.wkpath+'FreeEneHarmonic/F_J_FixRef_Formula298.'+str(i+1);
            self.GofQ4TCM.append(self.EfitPol(file,degree));
            # PARAMETER G(LIQUID,CR;0) 2.98140E+02 +24339.955-11.420225*T+2.37615E-21*T**7
            # ent_par G(Q,AL:AL:AL:AL:CU:SI) 298.15 9234.1184
            Tstring = str(self.GofQ4TCM[i][0]);
    ...
    def fitCALPHAD4TCMfR298(self):
        ''' prepare free energy CALPHAD polynomial as TCM syntax
            modified for mole of formula
            G  = A + B*T + C*T*LN(T) + D*T^2 + E*T^3  + F*T^-1
            Cp =         - C         - 2D*T  - 6E*T^2 - 2F*T^-2
        '''

        TCM = open('TCMparamCALPHADFixedRef298.txt','w');
        for i in range(self.nC):
            file = self.wkpath+'FreeEneHarmonic/F_J_FixRef_Formula298.'+str(i+1);
            Tstart=200
            popt,pcov = self.EfitTDB(file,Tstart);
            # PARAMETER G(LIQUID,CR;0) 2.98140E+02 +24339.955-11.420225*T+2.37615E-21*T**7
            # ent_par G(Q,AL:AL:AL:AL:CU:SI) 298.15 9234.1184
            Tstring = ' {0[0]} {0[1]:+}*T {0[2]:+}*T*LN(T) {0[3]:+}*T**2 {0[4]:+}*T**-1 '.format(popt)                
            Tstring = 'ent_par G('+self.phase+','+':'.join(list(self.Comb[i])[1:self.nM+1]).upper()+                 ':'+':'.join(self.Elfixed).upper()+') 298.15  \n'+ Tstring +                 '\n \n6000 \nN ;,,,,,,,Q  J/mol from VASP \n \n';
            TCM.write(Tstring+'\n');
            #print(Tstring+'\n')
        TCM.close();
    ...
    def HofQs(self):
        ...
        self.T0ofQ = [];
        ntot = np.sum(self.compositionTab[0][self.nElements:self.nElements*2]);
        Hfile=open(self.wkpath+'FreeEneHarmonic/Enthalpies.out','w');
        for el in self.Elements:
            #Total E @T0 provided in meV/atom
            self.T0ofEl.append(float(np.loadtxt(self.wkpath+'FreeEneHarmonic/T0_'+el)));
        for i in range(self.nC):
            #Total E0 of each Q provided in eV/atom (VASP --> murn --> converted from Hartree to eV/atom)
            #1st row of sum.out is skipped by using [i+1]
            self.T0ofQ.append(1000.0*float(np.array(self.file2dat(self.wkpath+self.name                                                                   +'/sum.out'))[i+1].split()[self.nM+2]));
            H = 0.0;
            for j in range(self.nElements):
            ...
            headerOfLine = ' '.join(s for s                         in self.file2dat(self.wkpath+self.name+'/sum.out')[i+1].split()[0:self.nM+3]);
    ...
    def TCMfromH(self):
        ...
        '''
        TCM = open(self.wkpath+'FreeEneHarmonic/TCMparamH.txt','w');
        file = np.array(self.file2dat(self.wkpath+'FreeEneHarmonic/Enthalpies.out'));
        for i in range(self.nC):
            hofQ4TCM = file[i].split()[-1];   #read the enthalpy of ith compound from last column of Enthalpies.out
            # PARAMETER G(LIQUID,CR;0) 2.98140E+02 +24339.955-11.420225*T+2.37615E-21*T**7
            # ent_par G(Q,AL:AL:AL:AL:CU:SI) 298.15 9234.1184
            Tstring = 'ent_par G('+self.phase+','+':'.join(list(self.Comb[i])[1:self.nM+1]).upper()+                 ':'+':'.join(self.Elfixed).upper()+') 298.15 '+ hofQ4TCM + ';,,,,,,,Q  J/mol from VASP'
            TCM.write(Tstring+'\n');
            #print(Tstring+'\n')
        TCM.close();
        return
...
import gzip, glob, json,itertools
# "/Users/firebird/Documents/Education/Projects/Q/9_imaginary/DATA/"
# "/nas/zendegani/Projects/Q/Calculations/9_imaginary/DATA"
tmp_path=op.join("/nas/zendegani/Projects/Q/Calculations/9_imaginary/testDATA", '')
...
Q = SiteFrac('Q', ['Al', 'Mg', 'Cu', 'Si'], [3,9,2,7], 2, ModeVerbose, ModeFile,wkpath=tmp_path);
...
# Generate composition table of each structure
Q.Composition();
print np.array(Q.compositionTab)


# In[48]:

# Calc free E or Enthalpy H of Qs
ForH = raw_input('Would you like to extract Enthalpy or Free energy \n   H: Enthalpy of Formation \n   F: Free Energy of Formation \n   R: Free Energy Fixed Ref   0K Polynomial fit\n   X: Free Energy Fixed Ref 298K Polynomial fit\n   C: Free Energy Fixed Ref 298K CALPHAD fit\n[No]: nothing?\n').upper()
if ForH == 'F':
    Q.EofQs();
    Q.fit4TCM();
elif ForH == 'H':
...
T=range(1,1000)
for i in Q.GofEl:
    plt.plot(T,np.polyder(i,2)(T)*-np.array(T)*96)
plt.ylim((0,30));plt.grid();
...
H_Qs=open(tmp_path+'FreeEneHarmonic/Enthalpies.out').readlines()


# In[71]:

for i in range(len(H_Qs)):
    H_Qs[i]=H_Qs[i].split()[-2]
