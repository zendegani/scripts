...
class SiteFrac(object):
    ...
    def EofQsFixedRef(self):
        ...
        for i in range(self.nC):
            ...
            for T in range(1000):
                ...
                F_meV_FixRef.write(str(T)+'   '+str(G/21.)+'\n');
                F_J_FixRef.write(str(T)+'   '+str(G*self.meV2J/21.)+'\n');
    ...
    def fit4TCM(self):
        ...
        for i in range(self.nC):
            ...
            Tstring = str(21*self.GofQ4TCM[i][0]);
            for d in range(degree):
                Tstring += '+' + str(21*self.GofQ4TCM[i][d+1]) + '*T**' + str(d+1)
...
if __name__ == '__main__':
        ...
        ForH = raw_input('Would you like to extract Enthalpy or Free energy (H:Enthalpy/F:FreeEnergy/R:FreeE-withFixedRef0K/X:FreeE-withFixedRef298K/[No]:nothing)?').upper()
        if ForH == 'F':
            Q.EofQs();
            Q.fit4TCM();
        elif ForH == 'H':
...
ax.set_ylabel('Free energies of formation '+ur'$F^{vib}_{harm}$'+' (kJ/mole of formula)', fontsize=18);
