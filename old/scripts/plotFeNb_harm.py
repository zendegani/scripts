...
plt.plot(NMmin[:,0]  ,NMmin[:,1]+NME0-shift, '--k', lw=1, label=r'$\mathrm{NM}\ V_{min}='+format(str(NMV0))+'\ \AA^3$')
plt.plot(DUDmin[:,0] ,DUDmin[:,1]+DUDE0-shift, '--r', lw=1, label=r'$\mathrm{DUD}\ V_{min}='+format(str(DUDV0))+'\ \AA^3$')
plt.plot(UUDmin[:,0] ,UUDmin[:,1]+UUDE0-shift, '-b', lw=2, label=r'$\mathrm{UUD}\ V_{min}='+format(str(Veq))+'\ \AA^3$')
plt.plot(NM157[:,0] ,NM157[:,1]+NME157-shift, '-k', lw=2, label=r'$\mathrm{NM}\ V='+format(str(Veq))+'\ \AA^3$')
plt.plot(DUD157[:,0],DUD157[:,1]+DUDE157-shift, '-r', lw=2, label=r'$\mathrm{DUD}\ V='+format(str(Veq))+'\ \AA^3$')
