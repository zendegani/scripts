...
if [ "$zip" = True ];then
    ...
    #echo "2-",einmehr:$einmehr
    #tmp1=`grep -A$einmehr POSITION $pfad` # | sed 's|POSITION.*||' | sed 's|.*--.*||g' | sed '/^ *$/d'`
    #echo "$tmp1"
...
#echo lines:$lines
#echo modulo:$modulo
[ "$modulo" != "0" ] && echo got lines:$lines: and number_atoms:$number_atoms: thus problem because some lines missing && exit
echo "$tmp" | awk '{print $4,$5,$6}' | tail -n$number_atoms
