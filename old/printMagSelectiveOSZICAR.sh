...
echored "THIS SCRIPT IS NOT FUNCTIONAL"
  ...
  case $opt in
    ...
    h)
      ...
      echo2 "Options: "\
              ...
              "        -m AXIS          [NOT implemented in OSZICAR version of this script] Extract the data of x,y or z direction of magnetic moments out of OUTCAR."\
...
folders=`getFoldersContain "${input}" $depth "OSZICAR*"`
echo -e "Folders are:\n$folders \n"
OSZICAR=OSZICAR
here=`pwd`
c=0
for i in $folders;do
    let c++
    cd $i
    result[$c]=`printMagnetizationTableOSZICAR`
    if [ $printTable == 'TRUE' ]; then
        echo "############### $i ##################"
        echo "${result[$c]}";
    fi
    echo "${result[$c]}" | parseMagnetizationTable2 > tmpOUT
    parseINCARmag INCAR $magDirec > tmpINC
    cd $here
done
