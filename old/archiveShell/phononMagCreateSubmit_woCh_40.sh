...
cp ../POTCAR .
cp ../KPOINTS .
qsub -N J_backF -pe impi_hydra 40 ~/scripts/submit.5.3.5.20cores_woCHG_NEWCONSTRAINS
...
for (( i=1; i<$ndir+1; i++ )); do 
  ...
  cp ../POTCAR .
  cp ../KPOINTS .
#  qsub -N J$i -pe impi_hydra 40 ~/scripts/submit.5.3.5.20cores_wCHG_NEWCONSTRAINS
  qsub -N J$i -pe impi_hydra 40 ~/scripts/submit.5.3.5.20cores_woCHG_NEWCONSTRAINS
  cd ..
done
